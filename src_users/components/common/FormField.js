...
export default class FormField extends React.Component {
    ...
    render() {
        ...
        if (doValidate) {
            return ( < FormGroup className = { className }
                validationState = {!meta.touched ? null : (meta.error ? 'error' : 'success') } > { this.content() } < FormControl.Feedback / >
                < HelpBlock > { meta.touched && meta.error ? meta.error : null } < /HelpBlock> < /FormGroup>
            );
        } else {
    ...
    content() {
        ...
        if ('other_theme' === theme) {
            ...
            // default theme: 2col
            return ( < Row >
                < Col sm = { 3 } > { label } < /Col> < Col sm = { 9 } > { this.field() } < /Col> < /Row>
            );
        }
    }
    ...
    field() {
        ...
        return ( < FormControl {...input }
            ...
            type = { type }
            placeholder = { placeholder } > { children } < /FormControl>
        );
    }
}
